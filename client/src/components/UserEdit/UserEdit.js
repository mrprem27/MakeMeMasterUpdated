...
export default function UserEdit({ user, setWantToEdit }) {
    ...
    const { viewModal, isTeacher, serverUrl } = useGlobalContext();
    const cp = useRef(null);
    const p = useRef(null);
    const file = useRef(null);
    const [image, setImage] = useState(user.dp ? `${serverUrl}/uploads/${user.dp}` : null);
    ...
    const submitHandler = async (e) => {
        ...
        else if ((password && password.length < 6) || (cpassword && cpassword.length < 6))
            viewModal("password should be min length of 6 ")
        else {
            try {
    ...
    return (
        <div className='edit-user-cover'> <button className='user-edit-btn' onClick={() =>
            setWantToEdit(false)
        } >❌</button>
            <div className="edit-user">
                <img src={image || img2} alt="this is an image" />
                <div className='signup-container edit-container'>
                    <form id="edit-form" onSubmit={(e) => e.preventDefault()}>
                        <div className="signup-input">
                            ...
                            <input type="text" name='cellno' onChange={(e) => setCellno(e.target.value)} placeholder='Mobile no.' value={cellno} />
