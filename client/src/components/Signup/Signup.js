...
export default function Signup() {
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
        console.log(email.current,fullname.current);
        const accDetails = {
        ...
        else if (!(accDetails.username && accDetails.username.length > 2))
            viewModal("Enter unique username for chat servers (minimum length of name should me 3)")
        ...
        else if (!(accDetails.password.length > 5))
            viewModal("Password length min shoulld be of 6 Char");
        else {
